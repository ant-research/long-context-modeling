            ...
            if self.is_master:
                ...
                if step % save_steps == 0 and step > 0:
                    try:
                        ...
                        #     os.path.remove(os.path.join(output_dir, f"model0_{step - save_steps}.bin"))
                        #     os.path.remove(os.path.join(output_dir, f"optimizer0_{step - save_steps}.pt"))
                        #     os.path.remove(os.path.join(output_dir, f"scheduler0_{step - save_steps}.pt"))
                        #     os.path.remove(os.path.join(output_dir, f'scaler0_{step - save_steps}.pt'))
                    except RuntimeError as e:
                ...
                if step % eval_steps == 0 and step > 0 and valid_dataloader is not None:
                    if eval_func is not None:
                        ...
                        model.train()
                        print(f'valid result: {ppl}, best: {best_ppl}')
                        if ppl < best_ppl:
                            best_ppl = ppl
...
if __name__ == '__main__':
    ...
    cmd.add_argument('--chunk_size', type=int, default=64)
    ...
    if args.finetune_path is not None:
        ...
        load_model(model, args.finetune_path, strict=False)
        args.mlm_prob = 0.0
    if args.disable_mlm:
        args.mlm_prob = 0.0
    ...
    # unused_parser_indices = "113 114 115 122 123 124"
    ...
    if args.valid_corpus_path is not None:
        if args.passkey_retrieval is None:
            valid_dataloader = create_slidewin_dataloader(args.valid_corpus_path, 
            ...
            valid_dataloader = create_passkey_dataloader(tokenizer,
    ...
    if global_rank == -1:
        dataloader = create_slidewin_dataloader(args.corpus_path, 
                                                args.batch_size, 
                                                args.max_seq_len, 
                                                args.epochs,
                                                seed=args.seed, 
        ...
        if args.passkey_retrieval is None:
            ...
            print(f'chunk retrieval: {not args.no_chunk_retrieval}')
    ...
    trainer.train(dataloader, args.mlm_prob, optimizer, 
                  ...
                  eval_func=eval_ppl if not args.passkey_retrieval else passkey_eval)