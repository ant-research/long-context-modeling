...
class ApplyRotaryEmb(torch.autograd.Function):
    ...
    def forward(ctx, x, cos, sin, interleaved=False, inplace=False):
        ...
        if len(cos.shape) == 2:
            cos_reshape = rearrange(cos[:seqlen], "s d -> s 1 d")
        elif len(cos.shape) == 3:
            cos_reshape = rearrange(cos[:, :seqlen, :], "b s d -> b s 1 d")
        if len(sin.shape) == 2:
            sin_reshape = rearrange(sin[:seqlen], "s d -> s 1 d")
        elif len(sin.shape) == 3:
            sin_reshape = rearrange(sin[:, :seqlen, :], "b s d -> b s 1 d")
