...
from model.flash_landmark_attention import fused_landmark_attention
...
class LlamaMLP(nn.Module):
    def __init__(
        ...
        assert self.hidden_act in ('silu', 'swish'), f"Fused SwiGLU requires silu / swish as activate function, but {config.hidden_act} found."
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        chunk_topk: Optional[int] = None,
        mem_freq: Optional[int] = None
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        # print(f'Attenion: use_cache={use_cache} use_flash={use_flash}')
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            if self.config.chunk_id is not None:
                with torch.no_grad():
                    is_mem = input_ids == self.config.chunk_id
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(torch.where(input_ids < 0, 0, input_ids))
        # embed positions
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        self.auto_insert_landmarks = config.auto_insert_landmarks
        self.always_use_flash = False
        print("mem_freq:",self.config.mem_freq)
        # print("use_flash:",self.config.use_flash, "always_use_flash",self.always_use_flash)
        # print("use_return_dict:",self.config.use_return_dict, "output_hidden_states:",self.config.output_hidden_states)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        # print(f"auto_insert_landmarks:{self.auto_insert_landmarks}")
        # print(f"config.mem_freq:{self.config.mem_freq}")
        ...
        if self.auto_insert_landmarks:
            ...
            input_ids = torch.cat((input_ids, input_ids.new_full((input_ids.shape[0], input_ids.shape[1], 1), self.config.chunk_id)), dim=-1)
        ...
        if max_chunk_length == 0:
            if chunk_topk is not None:
                # print(self.config.train_context_length, mem_freq, chunk_topk)
                max_chunk_length = self.config.train_context_length - self.config.train_context_length % (mem_freq + 1) - (chunk_topk + 1) * (mem_freq + 1)
                if max_chunk_length <= 0:
            ...
            # print(input_ids.shape)
            # if chunk_topk is not None:
            #     print(self.config.train_context_length, chunk_topk, self.config.train_context_length % (mem_freq + 1) ,(chunk_topk + 1) * (mem_freq + 1))
            # print(max_chunk_length)
        # print(f'chunk_topk={chunk_topk}')
        # print(f'max_chunk_length={max_chunk_length}')
        ...
        for step, idx in enumerate(range(0, input_ids.shape[1], window_len)):
            ...
            # print('LlamaModel input_ids.shape',input_ids[:, idx:idx + window_len].shape)
            # print("past_key_values:","None" if past_key_values==None else (len(past_key_values),len(past_key_values[0]),past_key_values[0][0].shape))
            if len(outputs.to_tuple()) > 1:
        ...
        if drop_last_logit_if_mem:
            is_any_mem = (input_ids[:, -1] == self.config.chunk_id).any()
            are_all_mem = (input_ids[:, -1] == self.config.chunk_id).all()
    ...
    def set_chunk_id(self, chunk_id):
        if self.config.chunk_id is not None:
            assert chunk_id == self.config.chunk_id, "Chanigng chunk_id can break the model. If you really intend to do this, manually disable this check"
        self.config.chunk_id = chunk_id

    def enable_landmark_insertion(self):
        self.config.train_context_length = self.config.train_context_length // self.config.mem_freq * (self.config.mem_freq+1)
    ...
    def prepare_inputs_for_generation(
        ...
        if mem_freq is not None:
            ...
            input_ids_with_mem = torch.cat(
                (
                    input_ids_with_mem, 
                    input_ids_with_mem.new_full((bsz, input_ids_with_mem.shape[1], 1), self.config.chunk_id)
                ), 
                dim=-1
            ).view(bsz, -1)
        ...
        # print("None" if past_key_values==None else (len(past_key_values),len(past_key_values[0]),past_key_values[0][0].shape))
        ...
        model_inputs.update(
            {
                ...
                "use_flash": use_flash,
                "chunk_topk": kwargs.get("chunk_topk"),
